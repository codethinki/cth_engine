...
struct AttachmentCollectionConfig {
    ...
    AttachmentCollectionConfig(size_t attachments_per_index, std::span<uint32_t const> attachment_indices, ImageConfig const& image_config,
    ...
    AttachmentCollectionConfig(size_t attachments, uint32_t index, ImageConfig const& image_config, AttachmentDescription const& description) :
        AttachmentCollectionConfig{attachments, std::vector{index}, image_config, description} {}
    ...
    static void debug_check(AttachmentCollectionConfig& config) {
        CTH_WARN(!std::ranges::is_sorted(config.attachmentIndices), "attachment indices are not sorted but will be in the collection") {}
    }
