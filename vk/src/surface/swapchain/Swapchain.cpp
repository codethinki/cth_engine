...
#include "vk/base/queue/CthPresentInfo.hpp"
#include "vk/render/cmd/CthCmdBuffer.hpp"
#include "vk/render/ctrl/CthFence.hpp"
#include "vk/render/ctrl/CthPipelineBarrier.hpp"
#include "vk/render/ctrl/CthSemaphore.hpp"
...
Swapchain::Swapchain(Core const& core, Queue const& present_queue, Surface const& surface, Config config) :
    _core(&core), _presentQueue(&present_queue), _surface{&surface}, _config{std::move(config)} { init(); }

Swapchain::Swapchain(Core const& core, Queue const& present_queue, Surface const& surface,
    Config const& config, VkExtent2D window_extent) : Swapchain{core, present_queue, surface, config} { create(window_extent); }
...
VkResult Swapchain::present(size_t in_flight_index) {
    ...
    CTH_CRITICAL(imageIndex == NO_IMAGE_INDEX, "no acquired vk_image available") { details->add("frame: ({})", in_flight_index); }
