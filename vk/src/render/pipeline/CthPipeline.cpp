...
void Pipeline::GraphicsConfig::setDefault(GraphicsConfig& config, PipelineVertexDescription const& vertex_description) {
    ...
    config.vertexInputInfo->vertexAttributeDescriptionCount = static_cast<uint32_t>(config.description.attributes.size());
    config.vertexInputInfo->pVertexAttributeDescriptions = config.description.attributes.data();

    config.vertexInputInfo->vertexBindingDescriptionCount = static_cast<uint32_t>(config.description.bindings.size());
...
Pipeline::GraphicsConfig Pipeline::GraphicsConfig::Default(PipelineVertexDescription const& vertex_description) {
