...
module cth.vk.render.rec.cmd.pool;
...
VkCommandBuffer CmdPool::newCmdBuffer(BufferType b_type) {
    debug_check(*this);
    debug_check_buffer_type(b_type);

    auto& buffers = _buffers[b_type];

    CTH_CRITICAL(buffers.empty(), "too many buffers requested, consider raising the max buffer limit") {}

    auto const handle = buffers.back();
    buffers.pop_back();

    return handle;
}

void CmdPool::returnCmdBuffer(BufferType b_type, VkCommandBuffer vk_buffer) {
    debug_check(*this);
    debug_check_buffer_type(b_type);

    auto& buffers = _buffers[b_type];
    auto const maxBuffers = _maxBuffers[b_type];
