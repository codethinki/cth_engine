...
Device::Device(Instance const& instance, PhysicalDevice const& physical_device, std::span<QueueFamilyProperties const> queues) :
...
void Device::create(std::span<QueueFamilyProperties const> queues) {
    optDestroy();
...
vector<uint32_t> Device::setUniqueFamilyIndices(span<QueueFamilyProperties const> queues) {
...
void Device::wrapQueues(span<uint32_t const> family_indices, span<QueueFamilyProperties const> queue_family_properties) {
    ...
    for(auto [familyIndex, familyProperties] : std::views::zip(family_indices, queue_family_properties)) {
        ...
        table()->vkGetDeviceQueue(_handle.get(), familyIndex, queueIndex, &ptr);
