...
module cth.vk.base.physical_device;
...
PhysicalDevice::PhysicalDevice(Instance const& instance, utils::PhysicalDeviceFeatures required_features,
    std::span<std::string const> required_extensions) : _instance{&instance}, _requiredFeatures{std::move(required_features)},
...
PhysicalDevice::PhysicalDevice(Instance const& instance, utils::PhysicalDeviceFeatures const& required_features,
    std::span<std::string const> required_extensions, vk::not_null<VkSurfaceKHR> surface, vk::not_null<VkPhysicalDevice> vk_device) : PhysicalDevice{
    instance,
...
std::optional<PhysicalDevice> PhysicalDevice::Create(Instance const& instance, vk::not_null<VkSurfaceKHR> surface,
    std::span<QueueFamilyProperties const> queues, std::span<std::string const> required_extensions,
...
void PhysicalDevice::create(vk::not_null<VkSurfaceKHR> surface, not_null<VkPhysicalDevice> vk_device) {
...
auto PhysicalDevice::AutoPick(Instance const& instance, std::span<QueueFamilyProperties const> queues, span<std::string const> required_extensions,
    ...
    for(auto& device : devices) {
        auto physicalDevice = Create(instance, surface.get(), queues, requiredExtensions, requiredFeatures, device);
...
auto PhysicalDevice::queueFamilyIndices(span<QueueFamilyProperties const> queues) const -> vector<uint32_t> {
    PhysicalDevice::debug_check(*this);

    vector<vector<uint32_t>> queueIndices{queues.size()};

    for(auto [properties, indices] : std::views::zip(queues, queueIndices)) {
        for(uint32_t index = 0; index < _queueFamilies.size(); ++index) {
            bool const support = (_queueFamilies[index].properties & properties) == properties;
...
bool PhysicalDevice::supportsQueueSet(span<QueueFamilyProperties const> family_properties) const {
    PhysicalDevice::debug_check(*this);

    return !queueFamilyIndices(family_properties).empty();
}
...
std::vector<QueueFamily> PhysicalDevice::getQueueFamilies(vk::not_null<VkSurfaceKHR> surface, vk::not_null<VkPhysicalDevice> vk_device) {
