module;
#include "lib/volk.hpp"
#include <cth/io/io_log.hpp>

export module cth.vk.res.descriptor.pool;


import cth.vk.constants;
import cth.vk.base.core;
import cth.vk.res.desciptor.set_layout;

import cth.io.log;
import cth.ptr;

import std;

export namespace cth::vk {
/**
 * @brief wrapper class for the VkDescriptorPool
 * @note the pool allocates the max_descriptor_sets instantly
 * @note all DescriptorSetLayout's ever used with the pool must be known at its creation
 */
class DescriptorPool {
public:
    struct Builder;
    /**
    * @param builder [layout, count] pairs -> limit for allocated sets per layout
    * @throws cth::vk::result_exception data: VkResult of vkCreateDescriptorPool()
    */
    DescriptorPool(Core const& core, Builder const& builder);
    ~DescriptorPool();


    VkDescriptorSet borrowSet(DescriptorSetLayout const& layout);

    /**
     * @brief returns a descriptor set
     * @attention does not recycle the set
     */
    void returnSet(VkDescriptorSet set);


    /**
     * @brief resets the pool -> resets all descriptor sets
     * @throws cth::vk::result_exception data: VkResult of vkResetDescriptorPool()
     */
    void reset();

private:
    struct SetLayoutEntry {
        void reset() { used = 0; }
        [[nodiscard]] VkDescriptorSet newVkSet() {
            CTH_CRITICAL(used >= span.size(), "out of descriptor sets") {}
            return span[used++];
        }
        [[nodiscard]] size_t size() const { return span.size(); }

        std::span<VkDescriptorSet> span{};
        uint32_t used = 0;
    };

    std::vector<VkDescriptorPoolSize> calcPoolSizes();

    void initSetEntries(std::map<DescriptorSetLayout const*, size_t> max_sets);
    /**
     * @throws cth::vk::result_exception data: VkResult of vkCreateDescriptorPool()
     */
    void create();

    /**
     * @throws cth::vk::result_exception data: VkResult of vkAllocateDescriptorSets()
     */
    void allocSets();


    cth::not_null<Core const*> _core;

    uint32_t _setCount;

    std::unordered_map<VkDescriptorSet, DescriptorSetLayout const*> _dispatchedSets;
    std::unordered_map<DescriptorSetLayout const*, std::vector<VkDescriptorSet>> _availableSets;

    move_ptr<VkDescriptorPool_T> _handle = VK_NULL_HANDLE;

    bool _reset = true;

public:
    [[nodiscard]] VkDescriptorPool get() const { return _handle.get(); }

    

    DescriptorPool(DescriptorPool const& other) = delete;
    DescriptorPool(DescriptorPool&& other) = delete;
    DescriptorPool& operator=(DescriptorPool const& other) = delete;
    DescriptorPool& operator=(DescriptorPool&& other) = delete;
};
}

namespace cth::vk {
struct DescriptorPool::Builder {
        Builder() = default;
        explicit Builder(std::map<DescriptorSetLayout const*, uint32_t> const& max_descriptor_sets) { addLayouts(max_descriptor_sets); }

        void addLayout(DescriptorSetLayout const& layout, uint32_t alloc_count);
        void addLayouts(std::map<DescriptorSetLayout const*, uint32_t> const& set_allocations);

        void removeLayout(DescriptorSetLayout const& layout, size_t amount = constants::WHOLE_SIZE);
        void removeLayouts(std::map<DescriptorSetLayout const*, uint32_t> const& set_allocations);

    private:
        std::map<DescriptorSetLayout const*, size_t> _maxDescriptorSets;

        friend DescriptorPool;

    public:
        [[nodiscard]] uint32_t setCount() const {
            return std::ranges::fold_left(_maxDescriptorSets | std::views::values, 0, [](auto sum, auto size) { return sum + size; });
        }
    };
}
