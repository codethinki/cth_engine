...
std::vector<VkWriteDescriptorSet> DescriptorSet::writes() {
    CTH_ERR(_handle == VK_NULL_HANDLE, "no descriptor set provided, call create() first")
...
namespace cth::vk {
DescriptorSet::Config::Config(DescriptorSetLayout const* layout) : _layout(layout) { init(layout); }
DescriptorSet::Config::Config(DescriptorSetLayout const* layout, std::span<Descriptor* const> descriptors, uint32_t binding_offset) : _layout(
    layout) {
    init(layout);
...
DescriptorSet::Config& DescriptorSet::Config::addDescriptor(Descriptor* descriptor, uint32_t binding, uint32_t arr_index) {
...
DescriptorSet::Config&
DescriptorSet::Config::addDescriptors(std::span<Descriptor* const> binding_descriptors, uint32_t binding, uint32_t arr_first) {
...
DescriptorSet::Config& DescriptorSet::Config::removeDescriptor(uint32_t binding, uint32_t arr_index) {
    _descriptors[binding][arr_index] = nullptr;
    return *this;
}
DescriptorSet::Config& DescriptorSet::Config::removeDescriptors(uint32_t binding, uint32_t arr_first, uint32_t count) {
...
void DescriptorSet::Config::init(DescriptorSetLayout const* layout) {
