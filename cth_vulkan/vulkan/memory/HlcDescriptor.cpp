...
HlcDescriptorSetLayout::HlcDescriptorSetLayout(
	...
	for(const auto& [snd, bnd] : bindings) setLayoutBindings.push_back(bnd);
	...
	if(vkCreateDescriptorSetLayout(hlc_device.device(), &descriptorSetLayoutInfo, nullptr, &descriptorSetLayout) !=
		VK_SUCCESS)
