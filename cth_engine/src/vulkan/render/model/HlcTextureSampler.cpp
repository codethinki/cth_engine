...
HlcTextureSampler::HlcTextureSampler(Device& device, const VkFilter filter, const VkSamplerAddressMode address_mode) : device{device} {
    ...
    if(vkCreateSampler(device.get(), &samplerInfo, nullptr, &textureSampler) != VK_SUCCESS) throw runtime_error(
        "HlcTextureSampler: failed to create texture sampler");
}
HlcTextureSampler::~HlcTextureSampler() { vkDestroySampler(device.get(), textureSampler, nullptr); }
