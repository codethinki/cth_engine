...
module cth.vk.render.sync.pipeline_barrier;
...
void ImageBarrier::transitionLayout(Image& image, VkImageLayout new_layout, VkAccessFlags src_access, VkAccessFlags dst_access,
    uint32_t first_mip_level, uint32_t mip_levels) {
    ...
    add(&image, ImageBarrier::Info::LayoutTransition(new_layout, src_access, dst_access, first_mip_level, mip_levels));
}
