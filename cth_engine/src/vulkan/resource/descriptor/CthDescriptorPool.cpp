...
void DescriptorPool::reset() {
    ...
    ranges::for_each(_descriptorSets, [](DescriptorSet* set) { set->deallocate(); });
...
void DescriptorPool::initSetEntries(const Builder& builder) {
    for(auto& [layout, count] : builder._maxDescriptorSets) {
        ...
        _allocatedSets[layout].span = span{&_vkSets[_vkSets.size() - count], count};
    }
}
...
void DescriptorPool::Builder::addLayouts(const unordered_map<const DescriptorSetLayout*, uint32_t>& set_allocations) {
...
void DescriptorPool::Builder::removeLayouts(const unordered_map<const DescriptorSetLayout*, uint32_t>& set_allocations) {
