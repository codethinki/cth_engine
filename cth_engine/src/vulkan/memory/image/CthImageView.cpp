...
ImageView::~ImageView() { vkDestroyImageView(device->get(), vkImageView, nullptr); }
...
void ImageView::create(const Config& config) {
    ...
    const auto createResult = vkCreateImageView(device->get(), &viewInfo, nullptr, &vkImageView);
