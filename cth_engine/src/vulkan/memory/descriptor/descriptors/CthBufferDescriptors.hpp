...
public:
    explicit BufferDescriptor(const VkDescriptorType type, const DefaultBuffer* buffer, const size_t descriptor_size = VK_WHOLE_SIZE,
        const size_t buffer_offset = 0) : Descriptor(type, descriptor_size, buffer_offset),
        vkDescriptorInfo(buffer->descriptorInfo(descriptor_size, buffer_offset)) {}
...
public:
    ...
    explicit UniformBufferDescriptor(const DefaultBuffer* buffer, const size_t descriptor_size,
        const size_t descriptor_offset) : BufferDescriptor(TYPE, buffer, descriptor_size, descriptor_offset) {}
};
