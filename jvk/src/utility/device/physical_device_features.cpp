...
PhysicalDeviceFeatures::PhysicalDeviceFeatures(VkPhysicalDeviceFeatures2 const& features) : _features(
    copy2(features)) {}

PhysicalDeviceFeatures::PhysicalDeviceFeatures(jvk::vk_not_null<VkPhysicalDevice> vk_device,
    PhysicalDeviceFeatures const& other) : PhysicalDeviceFeatures(other.features()) {
...
PhysicalDeviceFeatures::PhysicalDeviceFeatures(VkPhysicalDeviceFeatures2 const& a,
    VkPhysicalDeviceFeatures2 const& b) : PhysicalDeviceFeatures(a) {
    merge2(&b);
}

PhysicalDeviceFeatures::~PhysicalDeviceFeatures() { destroy(); }

auto PhysicalDeviceFeatures::supports(
    PhysicalDeviceFeatures const& required_features) const -> std::vector<std::variant<size_t,
    VkStructureType>> {
...
auto PhysicalDeviceFeatures::support(VkPhysicalDeviceFeatures2 const& available_features,
    VkPhysicalDeviceFeatures2 const& required_features) -> std::vector<std::variant<size_t,
    VkStructureType>> {
    ...
    while(required != nullptr) {
        ...
        if(available == nullptr || !checkSupport2(available, required))
            missingFeatures.emplace_back(
                required->sType);
...
bool PhysicalDeviceFeatures::checkSupport2(VkBaseOutStructure const* available_feature2,
    ...
    return std::ranges::equal(availableFlags, requiredFlags,
        [](VkBool32 available_flag, VkBool32 required_flag) {
...
void PhysicalDeviceFeatures::merge2(VkBaseOutStructure const* from, VkBaseOutStructure* to) {
    CTH_CRITICAL(from->sType != to->sType, "merging different types is not allowed, from({0}), to({1})",
        from->sType, to->sType) {}
...
auto PhysicalDeviceFeatures::copy2(
    VkPhysicalDeviceFeatures2 const& features) -> std::unique_ptr<VkPhysicalDeviceFeatures2> {
...
auto PhysicalDeviceFeatures::find2(VkPhysicalDeviceFeatures2 const* features,
    VkStructureType type) -> VkBaseOutStructure* {
