...
Pipeline::Pipeline(Core const& core, PipelineLayout const* pipeline_layout,
    GraphicsConfig const& config_info) : _core{&core} {
    create(config_info, pipeline_layout, nullptr);
}

Pipeline::Pipeline(Core const& core, Pipeline const* parent,
    GraphicsConfig const& config_info) : _core{&core} {
...
void Pipeline::bind(CmdBuffer const* cmd_buffer) const {
    _core->functions()->vkCmdBindPipeline(cmd_buffer->get(), VK_PIPELINE_BIND_POINT_GRAPHICS,
        _vkGraphicsPipeline);
}

void Pipeline::create(GraphicsConfig const& config_info, PipelineLayout const* pipeline_layout,
    Pipeline const* parent) {
    CTH_CRITICAL(pipeline_layout != nullptr && parent != nullptr,
        "something went wrong, cannot inherit and specify layout") {}

    CTH_CRITICAL(pipeline_layout == nullptr && parent == nullptr, "pipeline layout or parent invalid") {}

    CTH_STABLE_ERR(config_info.renderPass == VK_NULL_HANDLE && parent == nullptr,
        "renderPass missing in config_info")
    ...
    auto const createResult = _core->functions()->vkCreateGraphicsPipelines(_core->vkDevice(),
...
void Pipeline::GraphicsConfig::addShaderStage(Shader const* shader,
    ShaderSpecialization const* specialization_info,
...
void Pipeline::GraphicsConfig::setDefault(GraphicsConfig& config,
    PipelineVertexDescription const& vertex_description) {

    config.description = vertex_description;
    ...
    config.vertexInputInfo->vertexAttributeDescriptionCount = static_cast<uint32_t>(config.description.
        attributes.size());
    config.vertexInputInfo->pVertexAttributeDescriptions = config.description.attributes.data();

    config.vertexInputInfo->vertexBindingDescriptionCount = static_cast<uint32_t>(config.description.bindings.
        size());
    config.vertexInputInfo->pVertexBindingDescriptions = config.description.bindings.data();
...
Pipeline::GraphicsConfig Pipeline::GraphicsConfig::Default(
    PipelineVertexDescription const& vertex_description) {
    GraphicsConfig config;
    setDefault(config, vertex_description);
    return config;
}
}
