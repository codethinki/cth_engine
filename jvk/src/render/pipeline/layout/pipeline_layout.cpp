...
void PipelineLayout::create() {
    ...
    std::ranges::transform(_setLayouts, vkLayouts.begin(), [](DescriptorSetLayout const* layout) {
        return layout->get();
    });
    ...
    auto const result = _core->functions()->vkCreatePipelineLayout(_core->vkDevice(), &pipelineLayoutInfo,
        nullptr, &ptr);
...
namespace jvk {
PipelineLayout::Builder& PipelineLayout::Builder::addSetLayouts(std::span<DescriptorSetLayout* const> layouts,
...
PipelineLayout::Builder::addSetLayout(DescriptorSetLayout* layout, uint32_t location) {
...
std::vector<DescriptorSetLayout*> PipelineLayout::Builder::build(uint32_t max_bound_descriptor_sets) const {
    ...
    CTH_STABLE_ERR(_setLayouts.size() > max_bound_descriptor_sets,
        "device limits exceeded, too many locations") {
