...
DescriptorSetLayout::DescriptorSetLayout(Core const& core, Builder const& builder) : _core{&core},
    _vkBindings(builder.bindings()) { create(); }
...
void DescriptorSetLayout::create() {
    ...
    VkResult const result = _core->functions()->vkCreateDescriptorSetLayout(_core->vkDevice(),
        &descriptorSetLayoutInfo, nullptr, &ptr);
...
std::vector<VkDescriptorSetLayoutBinding> DescriptorSetLayout::Builder::bindings() const {
    ...
    CTH_CRITICAL(std::ranges::any_of(_bindings, [](const binding_t& binding) {
