#include "jvk/render/pass/framebuffer/fb_collection_config.hpp"

#include "jvk/render/pass/attachment/attachment_collection.hpp"

namespace jvk {

FramebufferCollectionConfig FramebufferCollectionConfig::Attachments(attachments_span_t collections,
    size_t max_framebuffers) {
...
size_t FramebufferCollectionConfig::evalFramebufferCount(attachments_span_t collections, size_t max) {
    return std::min(
        max,
        std::ranges::min(collections | std::views::transform([](AttachmentCollection const* ptr) {
            return ptr->per_attachment_size();
        }))
        );
}

FramebufferCollectionConfig::views_t FramebufferCollectionConfig::readViews(
    attachments_span_t const& collections, size_t framebuffers) {

    auto const attachments = 1 + std::ranges::max(
        collections | std::views::transform([](auto const* collection) { return collection->indices(); }) |
        std::views::join
        );
