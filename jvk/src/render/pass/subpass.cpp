...
namespace jvk {
namespace {
    auto attachment_ref_view(uint32_t index, auto const& rng) {
        return rng | std::views::transform([index](auto const* collection) {
            return collection->references(index);
        }) | std::views::join;
    }

    auto index_view(auto const& rng) {
        return rng | std::views::transform([](auto const* collection) { return collection->indices(); }) |
            std::views::join;
    }
}
...
Subpass::Subpass(
    ...
    auto inputAttachments = attachment_ref_view(index, input_attachments);
    auto colorAttachments = attachment_ref_view(index, color_attachments);
    auto resolveAttachments = attachment_ref_view(index, resolve_attachments);
...
void Subpass::setDepthAttachment(AttachmentCollection const& attachment) {
    ...
    CTH_CRITICAL(depthAttCount > 1, "there must at most be one depth attachment, currently: [{}]",
        depthAttCount) {}
    if(depthAttCount > 0) _depthAttachment = attachment.references(_index)[0];
}
