...
AttachmentCollection::AttachmentCollection(Core const& core, Config config) : _core{&core},
    _config{std::move(config)}, _images{total_size()} {
...
void AttachmentCollection::wrap(State state) {
    ...
    CTH_CRITICAL(
        images.size() != total_size(),
        "size() * attachments() [{}] image_handles required, image_handles: ({})",
...
void AttachmentCollection::createImages() {
    for(auto& image : _images) image = std::make_unique<Image>(*_core, _config.imageConfig, _extent);
}
...
size_t AttachmentCollection::index_of(size_t sub_index, size_t index_nr) const {
    CTH_CRITICAL(!cth::num::in(sub_index, 0, _config.attachmentsPerIndex),
        "sub index [{}] out of bounds [0, {})", sub_index, _config.attachmentsPerIndex){}
    CTH_CRITICAL(!cth::num::in(index_nr, 0, attachments()), "attachment_nr [{}] out of bounds [0, {})",
        index_nr, 0, attachments()) {}
...
ImageView const& AttachmentCollection::view(size_t sub_index, size_t att_index) const {
    return _views[index_of(sub_index, att_index)];
}

Image* AttachmentCollection::image(size_t sub_index, size_t att_index) const {
    return _images[index_of(sub_index, att_index)].get();
}


std::vector<VkAttachmentReference> AttachmentCollection::references(uint32_t subpass_index) const {
    CTH_CRITICAL(!_config.description.subpassLayouts.contains(subpass_index),
        "subpass index not present in attachment collection") {}


    return {
        std::from_range,
        _config.attachmentIndices | std::views::transform([this, subpass_index](auto index) {
            return VkAttachmentReference{.attachment = index,
                .layout = _config.description.subpassLayouts.at(subpass_index)};
        })
    };
}
}
