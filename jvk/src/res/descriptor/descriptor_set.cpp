...
#include "jvk/res/descriptor/descriptor.hpp"
...
namespace jvk {
DescriptorSet::DescriptorSet(Builder const& builder) : _layout(builder._layout),
    _descriptors(builder._descriptors) { copyInfos(); }
...
std::vector<VkWriteDescriptorSet> DescriptorSet::writes() {
    ...
    for(auto [binding, binding_descriptors] : _descriptors | std::views::enumerate) {
        ...
        for(auto const [index, descriptor] : binding_descriptors | std::views::enumerate) {
            ...
            if(type == InfoType::BUFFER) write.pBufferInfo = &_bufferInfos[_bufferInfos.size() - write.
                descriptorCount];
...
DescriptorSet::Builder::Builder(DescriptorSetLayout const* layout, std::span<Descriptor* const> descriptors,
    uint32_t binding_offset) : _layout(
    layout) {
    init(layout);
...
DescriptorSet::Builder& DescriptorSet::Builder::addDescriptor(Descriptor* descriptor, uint32_t binding,
    uint32_t arr_index) {

    CTH_CRITICAL(descriptor != nullptr && (descriptor->type() != _layout->bindingType(binding)),
        "matching descriptor and layout type required") {
        details->add("binding: {}", binding);
...
DescriptorSet::Builder::addDescriptors(std::span<Descriptor* const> binding_descriptors, uint32_t binding,
    uint32_t arr_first) {
    CTH_CRITICAL(_descriptors.size() + arr_first > _descriptors.size(),
        "out of range for layout size at binding") {
...
DescriptorSet::Builder& DescriptorSet::Builder::removeDescriptors(uint32_t binding, uint32_t arr_first,
    uint32_t count) {
    CTH_CRITICAL(arr_first + count > _descriptors[binding].size(),
        "out of ranger for layout size at binding") {
        details->add("binding: {0}, layout size: {1}", binding, _descriptors[binding].size());
