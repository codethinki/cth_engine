...
void DescriptorPool::writeSets(std::vector<DescriptorSet*> const& sets) {
    CTH_WARN(sets.empty(), "sets vector empty")
    ...
    std::ranges::for_each(sets, [this, &writes](DescriptorSet* set) {
        ...
        CTH_CRITICAL(set->written() || (set->_pool != nullptr && set->_pool != this),
            "set already registered in other pool") {}
    ...
    _core->functions()->vkUpdateDescriptorSets(_core->vkDevice(), static_cast<uint32_t>(writes.size()),
        writes.data(), 0, nullptr);
}

void DescriptorPool::reset() {
    VkResult const resetResult = _core->functions()->vkResetDescriptorPool(_core->vkDevice(), _handle.get(),
        0);
...
std::vector<VkDescriptorPoolSize> DescriptorPool::calcPoolSizes() {
    ...
    std::ranges::transform(maxDescriptorUses, poolSizes.begin(), [](auto const& pair) {
...
void DescriptorPool::allocSets() {
    ...
    VkResult const allocResult = _core->functions()->vkAllocateDescriptorSets(_core->vkDevice(), &allocInfo,
        _vkSets.data());
...
void DescriptorPool::Builder::addLayouts(
    std::unordered_map<DescriptorSetLayout const*, uint32_t> const& set_allocations) {
...
void DescriptorPool::Builder::removeLayouts(
    std::unordered_map<DescriptorSetLayout const*, uint32_t> const& set_allocations) {
    std::ranges::for_each(set_allocations, [this](auto const& pair) {
