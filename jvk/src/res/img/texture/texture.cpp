...
void Texture::blitMipLevels(CmdBuffer const& cmd_buffer, uint32_t first, uint32_t levels) {
    ...
    CTH_CRITICAL(_levelLayouts[first - 1] != VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
        "src layout not transfer src optimal") {}
    CTH_CRITICAL(
        std::ranges::any_of(
    ...
    ImageBarrier shaderBarrier{core(),
    ...
    for(uint32_t i = first; i < first + levels; i++) {
        if(i != first) {
            transitionLayout(toSrcBarrier, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_ACCESS_TRANSFER_WRITE_BIT,
                VK_ACCESS_TRANSFER_READ_BIT, i - 1, 1);
        ...
        transitionLayout(shaderBarrier, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
            VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT, i - 1, 1);
        shaderBarrier.execute(cmd_buffer);
        shaderBarrier.remove(this);
    }
    transitionLayout(shaderBarrier, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_ACCESS_TRANSFER_WRITE_BIT,
