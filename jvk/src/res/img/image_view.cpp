...
namespace jvk {
ImageView::ImageView(Core const& core, Config const& config) : _core{&core}, _config{config} {
    Core::debug_check(core);
}

ImageView::ImageView(Core const& core, Config const& config, Image const& image) : ImageView{core, config} {
    create(image);
}

ImageView::ImageView(Core const& core, Config const& config, State const& state) : ImageView{core, config} {
    wrap(state);
}
...
void ImageView::destroy() {
    ...
    auto const lambda = [table = _core->deviceTable(), vk_image_view = _handle.get()] {
        destroy(table, vk_image_view);
    };
...
VkImageViewCreateInfo ImageView::createViewInfo() const {
    auto const levels = _config.levelCount == 0 ? _image->mipLevels() - _config.baseMipLevel
        : _config.levelCount;

    auto const mask = _image->aspectMask();
    //config.range.aspectMask == VK_IMAGE_ASPECT_NONE ? _image->aspectMask() : config.range.aspectMask,
...
namespace jvk {
VkImageSubresourceRange ImageView::Config::range(uint32_t image_mip_levels,
