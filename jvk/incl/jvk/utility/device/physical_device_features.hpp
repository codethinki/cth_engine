...
public:
    ...
    explicit PhysicalDeviceFeatures(jvk::vk_not_null<VkPhysicalDevice> vk_device,
        PhysicalDeviceFeatures const& other);
     ...
     * @return unsupported features 
     */
    [[nodiscard]] static auto support(VkPhysicalDeviceFeatures2 const& available_features,
        VkPhysicalDeviceFeatures2 const& required_features)
...
private:
     ...
     */
    [[nodiscard]] static bool checkSupport2(VkBaseOutStructure const* available_feature2,
    ...
    [[nodiscard]] static std::unique_ptr<VkPhysicalDeviceFeatures2> copy2(
        VkPhysicalDeviceFeatures2 const& features);

    [[nodiscard]] static VkBaseOutStructure* find2(VkPhysicalDeviceFeatures2 const* features,
    ...
    template<class T> requires(std::same_as<VkPhysicalDeviceFeatures, cth::type::pure_t<T>>)
    [[nodiscard]] static auto to_span(T& features);
    template<class T> requires(std::same_as<VkBaseOutStructure, cth::type::pure_t<T>>)
...
public:
    ...
    static void debug_check(PhysicalDeviceFeatures const& features);
};
...
auto PhysicalDeviceFeatures::to_bool_args(T* feature2) {
    ...
    return std::span<bool_t>{reinterpret_cast<bool_t*>(feature2 + 1),
        PhysicalDeviceFeatures::flagCount2(feature2->sType)};
}
