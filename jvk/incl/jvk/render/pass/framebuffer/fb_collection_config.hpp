...
public:
     ...
     */
    static FramebufferCollectionConfig Attachments(attachments_span_t collections,
        size_t max_framebuffers = constants::ALL);
...
public:
    ...
    static void debug_check(FramebufferCollectionConfig const& config) {
        ...
        CTH_CRITICAL(std::ranges::any_of(config.imageViews, [](auto const* view) { return view == nullptr; }),
            "view must not be invalid") {}
    }
