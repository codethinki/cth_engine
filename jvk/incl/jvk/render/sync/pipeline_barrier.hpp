...
public:
    ...
    BufferBarrier(Core const& core, PipelineStages stages,
        std::unordered_map<BaseBuffer const*, Info> const& buffers);
...
public:
    ...
    PipelineBarrier(Core const& core, PipelineStages stages,
...
struct ImageBarrier::Info {
    ...
    VkImageAspectFlagBits aspectMask = constants::ASPECT_MASK_IGNORED;
    //Constants::ASPECT_MASK_IGNORED => vk_image default aspect
    ...
    static Info QueueTransition(PipelineAccess const& src, PipelineAccess const& dst) {
        return Info{.src = src, .dst = dst};
    }

    static Info QueueTransition(VkAccessFlags src_access, uint32_t src_queue_family_index,
