#pragma once
#include "jvk/res/buffer/base_buffer.hpp"

#include <volk.h>

#include <string>
#include <vector>


//TEMP modernize
namespace jvk {
struct DeviceTable;
class Device;

struct ShaderSpecialization {
    ShaderSpecialization(std::span<VkSpecializationMapEntry> entries, std::span<char> data);

private:
    VkSpecializationInfo _vkInfo;

public:
    [[nodiscard]] VkSpecializationInfo const* get() const { return &_vkInfo; }
};

class Shader {
public:
    using path_t = std::filesystem::path;

    static cxpr std::string_view DEFAULT_GLSLC_PATH = "glslc";

    /**
     *@throws jvk::result_exception result of @ref vkCreateShaderModule()
     */
    explicit Shader(Core const& core, VkShaderStageFlagBits stage, path_t spv_path);
    /**
     *@throws jvk::result_exception result of @ref vkCreateShaderModule()
     */
    explicit Shader(Core const& core, VkShaderStageFlagBits stage, std::span<char const> spv);

    ~Shader();

    static void destroy(DeviceTable table, VkShaderModule vk_shader);

private:
    std::vector<char> loadSpv();
    void create(std::span<char const> spv);
    void destroy();
    void optDestroy() { if(created()) destroy(); }
    void reset();


    static std::string filename(std::string_view path) {
        return std::filesystem::path{path.data()}.filename().string();
    }
#ifndef _FINAL
    void compile(path_t const& glsl_path, path_t const& compiler_path, std::string_view flags = "-O") const;
#endif

    not_null<Core const*> _core;
    VkShaderStageFlagBits _vkStage;
    path_t _spvPath;

    move_ptr<VkShaderModule_T> _handle = VK_NULL_HANDLE;

public:
#ifndef _FINAL
    /**
    *@throws jvk::result_exception result of @ref vkCreateShaderModule()
    */
    explicit Shader(Core const& core, VkShaderStageFlagBits stages, path_t spv_path, path_t const& glsl_path,
        path_t const& compiler_path = DEFAULT_GLSLC_PATH);
#endif
    [[nodiscard]] bool created() const { return _handle != VK_NULL_HANDLE; }
    [[nodiscard]] VkShaderModule module() const { return _handle.get(); }
    [[nodiscard]] VkShaderStageFlagBits stage() const { return _vkStage; }

    Shader(Shader const& other) = delete;
    Shader(Shader&& other) = delete;
    Shader& operator=(Shader const& other) = delete;
    Shader& operator=(Shader&& other) = delete;
};
} // namespace cth