...
public:
    ...
    * @throws jvk::result_exception data: VkResult of @ref vkCreateDescriptorSetLayout()
    */
...
public:
    ...
    [[nodiscard]] VkDescriptorSetLayoutBinding binding(uint32_t binding) const {
        return _vkBindings[binding];
    }

    [[nodiscard]] VkDescriptorType bindingType(uint32_t binding) const {
        return _vkBindings[binding].descriptorType;
    }
...
struct DescriptorSetLayout::Builder {
    Builder() = default;
    Builder& addBinding(uint32_t binding, VkDescriptorType type, VkShaderStageFlags flags,
        uint32_t count = 1);
