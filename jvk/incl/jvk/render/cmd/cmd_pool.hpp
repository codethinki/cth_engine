...
template<class T>
concept cmd_buffer_t = cth::type::is_any_of<T, PrimaryCmdBuffer, SecondaryCmdBuffer>;
...
public:
     ...
     * @note calls @ref optDestroy()
     * @throws jvk::result_exception VkResult of vkCreateCommandPool()
     * @throws jvk::result_exception VkResult of vkAllocateCommandBuffers()
     */
    void create();
...
private:
     ...
     * @throws jvk::result_exception VkResult of vkAllocateCommandBuffers() (for primary or secondary buffers)
     */
    void alloc();
    ...
    constexpr static VkCommandBufferLevel to_buffer_level(BufferType type) {
        return static_cast<VkCommandBufferLevel>(type);
    }
...
public:
    ...
    static void debug_check_handle(jvk::vk_not_null<VkCommandPool> vk_pool);
...
struct CmdPool::Config {
    ...
    static Config Default(uint32_t queue_family_index, uint32_t max_primary_buffers,
...
struct CmdPool::State {
    jvk::vk_not_null<VkCommandPool> vkPool;
    std::vector<jvk::vk_not_null<VkCommandBuffer>> vkPrimaryBuffers;
    std::vector<jvk::vk_not_null<VkCommandBuffer>> vkSecondaryBuffers;
};
}
...
inline void CmdPool::debug_check_handle([[maybe_unused]] jvk::vk_not_null<VkCommandPool> vk_pool) {}
...
inline void CmdPool::debug_check_unused(CmdPool const& pool) {
    ...
    CTH_CRITICAL(
        ...
        details->add("missing primary buffers: {}",
            maxBuffers[BUFFER_TYPE_PRIMARY] - buffers[BUFFER_TYPE_PRIMARY].size());
        details->add("missing secondary buffers: {}",
            maxBuffers[BUFFER_TYPE_SECONDARY] - buffers[BUFFER_TYPE_SECONDARY].size());
    }
}
