...
public:
     ...
     * @throws jvk::result_exception result of vkBindImageMemory if not bound
     ...
     * @throws jvk::result_exception result of vkCreateImage
     * @throws jvk::result_exception result of vkBindImageMemory
    ...
    */
     ...
     */
    void copy(CmdBuffer const& cmd_buffer, BaseBuffer const& src_buffer, size_t src_offset = 0,
     ...
     */
    void transitionLayout(CmdBuffer const& cmd_buffer, VkImageLayout new_layout, uint32_t first_mip_level = 0,
        uint32_t mip_levels = constants::ALL);
    /**
    * @brief adds the transition to the pipeline barrier
    * @param mip_levels (Constants::ALL => all remaining)
    */
    void transitionLayout(ImageBarrier& barrier, VkImageLayout new_layout, VkAccessFlags src_access,
...
public:
    ...
    static void debug_check_handle(jvk::vk_not_null<VkImage> vk_image);
    static void debug_require_memory(Image const& image);
};
...
namespace jvk {
struct Image::State {
    State(VkExtent2D extent, jvk::vk_not_null<VkImage> vk_image, bool const bound,
    ...
    VkExtent2D extent;
    jvk::vk_not_null<VkImage> vkImage;
    /**
   ...
   */
     ...
     */
    std::unique_ptr<Memory> memory;
    std::vector<VkImageLayout> levelLayouts{};
    // levelLayouts.size() < mipLevels => remaining levels are config.initialLayout
...
inline void Image::debug_check_handle([[maybe_unused]] jvk::vk_not_null<VkImage> vk_image) {}

inline void Image::debug_require_memory(Image const& image) {
    CTH_CRITICAL(
        image.memory() == nullptr,
        "image memory required, either wrapped without memory or ImageConfig::memoryProperties not set"
    ){}
}
}
