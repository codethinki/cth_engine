#pragma once
#include "../buffer/base_buffer.hpp"
...
public:
    ...
    struct Builder {
         ...
         */
        explicit Builder(DescriptorSetLayout const* layout, std::span<Descriptor* const> descriptors,
            uint32_t binding_offset = 0);

        Builder& addDescriptor(Descriptor* descriptor, uint32_t binding, uint32_t arr_index = 0);
        Builder& addDescriptors(std::span<Descriptor* const> binding_descriptors, uint32_t binding,
            uint32_t arr_first);
